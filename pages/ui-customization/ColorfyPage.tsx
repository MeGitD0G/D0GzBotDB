...
const generateShades = (hexColor: string, count: number = 9): { [key: number]: string } => {
  ...
  try {
    if (!hexColor || !hexColor.startsWith('#') || (hexColor.length !== 4 && hexColor.length !== 7)) {
      console.warn("Invalid hexColor for generateShades:", hexColor, "Using gray fallback.");
      for (let i = 0; i < count; i++) shades[(i + 1) * 100] = '#808080'; // Default gray
      return shades;
    }
...
// Function to determine contrasting text color for a given background
const getContrastingTextColor = (bgColor: string): string => {
    if (!bgColor || !bgColor.startsWith('#') || (bgColor.length !== 4 && bgColor.length !== 7)) {
        console.warn("Invalid bgColor for getContrastingTextColor:", bgColor, "Falling back to black.");
        return '#000000'; // Fallback for completely invalid format
    }
    try {
        let r, g, b;
        ...
        if (isNaN(r) || isNaN(g) || isNaN(b)) {
             console.warn("Parsed NaN for RGB in getContrastingTextColor from:", bgColor, "Falling back to black.");
             return '#000000'; // Fallback if parsing results in NaN
        }
        
        const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
        return luminance > 0.5 ? '#000000' : '#FFFFFF'; // Dark text on light bg, light text on dark bg
    } catch (e) {
        console.error("Error in getContrastingTextColor for", bgColor, e, "Falling back to black.");
        return '#000000'; // Fallback on any other error
    }
};
...
const ColorfyPage: React.FC = () => {
  const initialPalette: ColorPalette = {
    ...
    background: '#f3f4f6', // neutral-100 (light)
    text: '#111827',       // gray-900 (light)
  };

  const [palette, setPalette] = useState<ColorPalette>(initialPalette);
  const [isSaving, setIsSaving] = useState(false);
  const [darkMode, setDarkMode] = useState(false); // For preview container's dark mode state

  useEffect(() => {
     // Initialize darkMode for the preview container based on system preference or localStorage global theme
     const isSystemDark = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
     const localTheme = localStorage.getItem('theme'); // Global theme for the app
     const currentAppIsDark = localTheme === 'dark' || (!localTheme && isSystemDark);
     setDarkMode(currentAppIsDark); // Set preview dark mode to match app's current theme initially
     ...
     if (savedPalette) {
       ...
       // Set initial background/text based on current app theme if no saved palette
       setPalette(prev => ({
         ...prev,
         background: currentAppIsDark ? '#0f172a' : '#f8fafc', // Tailwind neutral-900 / neutral-50
         text: currentAppIsDark ? '#f1f5f9' : '#0f172a' // Tailwind neutral-100 / neutral-900
       }));
     }
  }, []);
  ...
  useEffect(() => {
    ...
    Object.entries(palette).forEach(([name, value]) => {
      if(value) { 
        ...
        // Example for generating and setting shades for primary, secondary, accent colors
  ...
  const handleResetPalette = () => {
    ...
    setPalette({
        ...initialPalette, 
        background: currentAppIsDark ? '#0f172a' : '#f8fafc', 
        text: currentAppIsDark ? '#f1f5f9' : '#0f172a'
    });
  };
    ...
    <Card title="Colorfy - Customize Your Theme Colors">
      ...
      <Card title="Live Preview" className="mt-8">
        ...
        <div className="mt-2 text-right">
          <Button 
            onClick={() => {
              const newPreviewMode = !darkMode;
              setDarkMode(newPreviewMode);
              // If no custom palette is saved, sync preview's bg/text with the new mode's defaults
              if (!localStorage.getItem('colorfyPalette')) {
                  setPalette(prev => ({
                      ...prev,
                      background: newPreviewMode ? '#0f172a' : '#f8fafc', // Default dark/light bg
                      text: newPreviewMode ? '#f1f5f9' : '#0f172a'       // Default dark/light text
                  }));
              }
            }} 
            variant="outline" 
